...
# 对 "Time Until the Failure Occur(in days)" 处理，大于 1825 的设置为 1825
data["Time Until the Failure Occur(in days)"] = data["Time Until the Failure Occur(in days)"].clip(upper=1825)
...
# 计算每个 bag 的失败比例，作为 bag 的 label
bag_failure_ratio = (
    bagged_data.groupby("Bag")["Treatments"]
    .mean()
    .reset_index(name="Bag_Label")
)

# 合并失败比例到主数据框
bagged_data = bagged_data.merge(bag_failure_ratio, on="Bag", how="left")
bagged_data.rename(columns={"Bag_Label": "bag_labels"}, inplace=True)

# 只保留筛选的特征和必要字段
output_columns = selected_features + ["Time Until the Failure Occur(in days)", "label", "bag_labels", "bag_names"]
bagged_data = bagged_data[output_columns]
...
# 计算并保存 pfs_avg.csv
pfs_avg = bagged_data.groupby("bag_names")["Time Until the Failure Occur(in days)"].mean().reset_index(name="PFS_Avg")
pfs_avg.to_csv("pfs_avg.csv", index=False)
print("pfs_avg.csv 文件已成功保存。")

# 计算并保存 ma_avg.csv，使用 Bone density (quality) 作为替代指标
ma_avg = bagged_data.groupby("bag_names")["Bone density (quality)"].mean().reset_index(name="MA_Avg")
